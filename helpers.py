...
def get_table_and_function_names_from_sql(context, sql):
    ...
    while queries:
        ...
        table_names.extend(re.findall(r'\bpg_\w+', sql))
...
def check_query_blacklist(sql):                                                                                                                                         
    ...
    blacklist_keywords =  ['current_catalog', 'current_role', 
                           'current_user', 'session_user', 'user']
                                                                                                                                                                    
    blacklisted = []                                                                                                                                                
    for token in tokens:                                                                                                                                            
        # blacklist if query token have blacklisted keywords.                                                                                                             
        if token.value.lower() in  blacklist_keywords:
            blacklisted.append(token)     
       # blacklist column  keywords  eg. 'SELECT (current_user)'                                                                                                                         
        elif isinstance(token, sqlparse.sql.Parenthesis): 
            column_list = extract_definitions(token)
            for column in  column_list:
              if column[0].value.lower() in blacklist_keywords:
                  blacklisted.append(column[0])  
        # Disallow to create a new column with blacklisted func  eg. 'SELECT current_user as "user" ...' 
        elif isinstance(token, sqlparse.sql.IdentifierList): 
            for identifier in token.get_identifiers(): 
                if identifier.value.lower() in blacklist_keywords: 
