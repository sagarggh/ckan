...
def upsert_data(context, data_dict):
    ...
    if method == _INSERT:
        ...
        try:
            log.info('=============== insert method sql execute date format - 1039================>') 
            context['connection'].execute('SET DATESTYLE TO ISO, DMY;')
            log.info('=============== insert method sql executet - 1041 ================>')
            context['connection'].execute(sql_string, rows)
        except sqlalchemy.exc.DataError:
            log.info('=============== insert method DataError 1044 ================>')  
            raise InvalidDataError(
                toolkit._("The data was invalid (for example: a numeric value "
                          "is out of range or was inserted into a text field)."
                          ))
        except sqlalchemy.exc.DatabaseError as err:
            log.info('=============== insert method data DatabaseError - 1050 ================>')   
            raise ValidationError(
                {u'records': [_programming_error_summary(err)]})

    elif method in [_UPDATE, _UPSERT]:
        log.info('=============== update upsert method - 1055 ================>')  
        unique_keys = _get_unique_key(context, data_dict)
        ...
        for num, record in enumerate(records):
            ...
            if method == _UPDATE:
                log.info('=============== upsert update method - 1097 ================>')
                sql_string = u'''
                    UPDATE "{res_id}"
                ...
                try:
                    log.info('=============== update method sql execute - 1114 ================>')  
                    results = context['connection'].execute(
                        sql_string, used_values + unique_values)
                except sqlalchemy.exc.DatabaseError as err:
                    log.info('=============== update method exception error - 1118 ================>')   
                    raise ValidationError({
                        u'records': [_programming_error_summary(err)],
                        u'_records_row': num})
                ...
                try:
                    log.info('=============== upsert method sql execute - 1151  ================>')  
                    context['connection'].execute(
                        sql_string,
                        (used_values + unique_values) * 2)
                except sqlalchemy.exc.DatabaseError as err:
                    log.info('=============== upsert method exception error - 1156 ================>')   
                    raise ValidationError({
                        u'records': [_programming_error_summary(err)],
                        u'_records_row': num})
                except Exception as e:
                    log.info('=============== upsert method  blind exception 1161 ================>')  
                    raise 
...
def upsert(context, data_dict):
    ...
    try:
        ...
        log.info('========================= Start upsert result - 1441 =====================>')
        upsert_data(context, data_dict)
        trans.commit()
        return _unrename_json_field(data_dict)
    except IntegrityError as e:
        if e.orig.pgcode == _PG_ERR_CODE['unique_violation']:
            log.info('================ upsert return validation error - 1447================>')
	    raise ValidationError({
	...
	log.info('======================== upsert IntegrityError after validation error - 1456  =====================>')
        raise
    except DataError as e:
        log.info('================ upsert return data errror - 1459 ================>')
        raise ValidationError({
            'data': e.message,
            'info': {
                'orig': [str(e.orig)]
        ...
        log.info('================ upsert return DBAPIError errror - 1466  ================>')    
        ...
        log.info('================ upsert loggin  blind exception - 1473 ================>')
        trans.rollback()
        raise
    finally:
        log.info('================ upsert logging finally  1477  ================>')                   
...
def search_sql(context, data_dict):
    ...
    try:
        ...
        if(datastore_helpers.check_query_blacklist(sql)):
	          raise toolkit.NotAuthorized({                                                                                                                           
                'permissions': ['Not authorized to access system tables or function']                                                                                           
            }) 
