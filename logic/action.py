...
def datastore_upsert(context, data_dict):
    ...
    **Results:**
    ...
    records = data_dict.pop('records', None)
    data_dict, errors = _validate(data_dict, schema, context)
    log.info('================== Start upsert payload print -  243 ==========>') 
    log.info(data_dict)
    log.info(records)
    log.info('================== End upsert payload print - 245 ==========>') 
    if records:
        data_dict['records'] = records
    if errors:
        raise p.toolkit.ValidationError(errors)
    log.info('================== upsert before check access - 250 ==========>')
    ...
    log.info('================== Start upsert result - 256  ==========>')
    log.info(result)
    log.info('================== End upsert result  267 ==========>')
    result.pop('id', None)
    result.pop('connection_url', None)
    return result
